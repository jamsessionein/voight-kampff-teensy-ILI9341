...
//  Implemented some basic color blending functionality - this struct is just a format to help keep things organized.
...
{
  public:
	ILI9341_t3(uint8_t _CS, uint8_t _DC, uint8_t _RST = 255, uint8_t _MOSI=11, uint8_t _SCLK=13, uint8_t _MISO=12, uint8_t _LCD=6);
	void begin(void);
  ...
  //  Exact same version of the above, but can return an RGB data type for further work.
  RGB color565toRGB(uint16_t color) {
  		uint8_t r = (color>>8)&0x00F8;
  		uint8_t g = (color>>3)&0x00FC;
  		uint8_t b = (color<<3)&0x00F8;
      RGB colorConverted = {r,g,b};
  ...
  uint16_t blendTwoColors(uint16_t colorStart, uint16_t colorEnd, int opacity);
  ...
  //  Changes made to the drawBitmap function so that it now accepts an optional opacity argument in percentage; default is 100%.
	void drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, int opacity = 100);

	void flipHorizontal(bool state); // Seems buggy. Be careful.
